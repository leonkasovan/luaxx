...
/* luaCSV http://mysite.mweb.co.za/residents/sdonovan/lua/luaCSV.zip
...
static int l_reader (lua_State *L) {
	...
	char delim = *luaL_optstring(L,2,",");
    boolean headers = optboolean(L,3,TRUE);
    boolean try_convert = optboolean(L,4,TRUE);
...
static int l_parse(lua_State *L) {
    lua_newtable(L);
    if (! parse_csv_line(L,lua_tostring(L,1),*luaL_optstring(L,2,","),NULL))
        return 0;
    return 1;
}
