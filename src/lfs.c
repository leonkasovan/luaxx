...
int lfs_win32_lstat(const char *path, STAT_STRUCT * buffer)
{
  ...
  WCHAR lpath[LFS_MAXPATHLEN+1];

  mbstowcs(lpath, path, LFS_MAXPATHLEN);
  if (GetFileAttributesEx(lpath, GetFileExInfoStandard, &win32buffer)) {
...
static int lfs_lock_dir(lua_State * L)
{
  ...
  strcpy(ln, path);
  strcat(ln, lockfile);
  mbstowcs(lln, ln, LFS_MAXPATHLEN);
  fd = CreateFile(lln, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
...
static int make_link(lua_State * L)
{
  ...
  WCHAR loldpath[LFS_MAXPATHLEN+1];
  WCHAR lnewpath[LFS_MAXPATHLEN+1];
  ...
  result = symbolic ? CreateSymbolicLink(lnewpath, loldpath, is_dir)
      : CreateHardLink(lnewpath, loldpath, NULL);
...
static int push_link_target(lua_State * L)
{
  ...
  while (!ok) {
    ...
    target = target2;
#ifdef _WIN32
	mbstowcs(ltarget, target, LFS_MAXPATHLEN);
    tsize = GetFinalPathNameByHandle(h, ltarget, size, FILE_NAME_OPENED);
#else
